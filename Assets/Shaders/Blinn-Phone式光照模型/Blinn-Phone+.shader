Shader "Study/Blinn-Phone+"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //计算Blinn-Phone高光反射模型 颜色 函数（传入世界空间的法线和顶点）
            fixed3 getSpecularFColor(in float3 wNormal,in float3 wPos)
            {
                ...
                fixed3 color = _SpecularColor.rgb * _LightColor0.rgb * pow(max(0,dot(halfA,normalize(wNormal))),_SpecularNum);
